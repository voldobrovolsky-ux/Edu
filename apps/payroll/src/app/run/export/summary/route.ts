...
import { buildPersonSummaryCsv } from "@/lib/payroll/payroll-export-csv";
...
export async function GET(req: NextRequest) {
  ...
  const csv = buildPersonSummaryCsv(run.period.year, run.period.month, lines);
  const filename = `payroll-summary-${run.period.year}-${String(run.period.month).padStart(2, "0")}.csv`;
